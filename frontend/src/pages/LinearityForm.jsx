...
export default function LinearityForm() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    const x = rows.map(r => parseFloat(r.x));
    const y = rows.map(r => parseFloat(r.y));
    if (x.some(isNaN) || y.some(isNaN)) {
      alert('Please fill all X and Y as numbers.');
      return;
    }
    setLoading(true);
    try {
      const { data } = await client.post('/linearity/', { ...metadata, x, y });
      setResult(data);
    } catch (err) {
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h4 className="font-medium">Observations (Concentration vs Area)</h4>
            ...
            {rows.map((r, i) => (
              <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-3">
                <input type="number" step="any" placeholder="X (Concentration)" value={r.x}
                       onChange={(e) => onRow(i, 'x', e.target.value)}
                       className="px-3 py-2 border rounded-lg focus:ring-2 focus:ring-sky-500" required />
                <input type="number" step="any" placeholder="Y (Area)" value={r.y}
                       onChange={(e) => onRow(i, 'y', e.target.value)}
                       className="px-3 py-2 border rounded-lg focus:ring-2 focus:ring-sky-500" required />
                {rows.length > 1 && (
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => { setRows([{ x: '', y: '' }]); setResult(null); }}
        ...
        <Card title="Results">
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-1">
              <div><span className="text-slate-500">R²:</span> <span className="font-medium">{result.r_squared}</span></div>
              <div><span className="text-slate-500">Slope:</span> <span className="font-medium">{result.slope}</span></div>
              <div><span className="text-slate-500">Intercept:</span> <span className="font-medium">{result.intercept}</span></div>
              <div><span className="text-slate-500">Residual SD:</span> <span className="font-medium">{result.residual_std_dev}</span></div>
              <div className="mt-2">
            ...
            <div className="min-h-[260px]">
              <Line
                data={{
                  labels: result.chart_data.map(p => p.x),
                  datasets: [
                    {
                      label: 'Area vs Concentration',
                      data: result.chart_data.map(p => p.y),
